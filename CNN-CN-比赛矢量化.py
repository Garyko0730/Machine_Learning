...
class Conv3x3:# 使用3x3滤波器的卷积层
    ...
    def forward(self, input):
        ...
        # 矢量化实现
        padded_input = np.pad(input, ((1, 1), (1, 1)), mode='constant', constant_values=0)
        input_view = np.lib.stride_tricks.as_strided(padded_input, 
                                                     shape=(h - 2, w - 2, 3, 3), 
                                                     strides=2 * padded_input.strides)
        output = np.tensordot(input_view, self.filters, axes=([2, 3], [1, 2]))
    ...
    def backprop(self, d_L_d_out, learn_rate):
       ...
       # '''# 初始化一组为 0 的 gradient，3x3x8'''
        ...
        # 矢量化实现
        # 创建输入的3x3区域的视图
        padded_input = np.pad(self.last_input, ((1, 1), (1, 1)), mode='constant', constant_values=0)
        input_view = np.lib.stride_tricks.as_strided(padded_input, 
                                                     shape=(d_L_d_out.shape[0], d_L_d_out.shape[1], 3, 3), 
                                                     strides=2 * padded_input.strides)

        # 计算关于滤波器的梯度
        d_L_d_filters = np.tensordot(d_L_d_out, input_view, axes=([0, 1], [0, 1]))
...
class MaxPool2:
    ...
    def forward(self, input):
        ...
        # 矢量化实现
        reshaped_input = input.reshape(new_h, 2, new_w, 2, num_filters)
        output = reshaped_input.max(axis=1).max(axis=2)
        return output
    ...
    def backprop(self, d_L_d_out):
        ...
        # 改变输入形状以方便找到最大值的位置
        reshaped_input = self.last_input.reshape(d_L_d_out.shape[0], 2, d_L_d_out.shape[1], 2, d_L_d_out.shape[2])

        # 找到每个2x2区域的最大值位置
        max_indices = reshaped_input.argmax(axis=1).argmax(axis=2)
    
        # 为每个区域创建布尔掩码
        mask = (reshaped_input == reshaped_input.max(axis=1, keepdims=True).max(axis=2, keepdims=True))
    
        # 扩展 d_L_d_out 的形状以匹配输入形状
        d_L_d_out_expanded = d_L_d_out[:, None, :, None, :]
    
        # 将 d_L_d_out 梯度分配到对应的最大值位置
        d_L_d_input = mask * d_L_d_out_expanded
...
class Softmax:
    ...
    def backprop(self, d_L_d_out, learn_rate,lambda_reg=0):
        '''
        执行softmax层的反向传播。
        返回这一层输入的损失梯度。
        - d_L_d_out是这一层输出的损失梯度。
        ...
        for i, gradient in enumerate(d_L_d_out):
            ...
            # 更新权重和偏差
            # L2 正则化项
            regularization_term = lambda_reg * self.weights
            self.weights -= learn_rate * (d_L_d_w + regularization_term)
...
# 训练CNN N个周期
for epoch in range(15):#训练次数参数设置
    print('\n--- 第%d周期 ---' % (epoch + 1))
